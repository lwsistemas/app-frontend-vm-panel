...
import {
    Play, Square, RotateCcw, Monitor, Cpu, MemoryStick, HardDrive, Loader2,
} from 'lucide-react';
...
export default function VmCards() {
    ...
    function statusBadge(status) {
        ...
        return (<span
            className={`px-2 py-1 rounded text-xs font-medium ${map[status] || map.UNKNOWN}`}
        >
                {status}
            </span>);
    }
    ...
    if (loading) {
        return (<div className="flex items-center gap-2 text-sm text-slate-400">
            ...
            >
                ...
                <div className="grid grid-cols-3 gap-3 text-xs text-slate-400">
                    ...
                    <Spec
                        ...
                        value={vm.memory_mb ? `${vm.memory_mb} MB` : '—'}
                    />
                    <Spec
                        icon={<HardDrive size={14}/>}
                        value={vm.disk_gb ? `${vm.disk_gb} GB` : '—'}
                    />
                </div>
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-slate-800">
                    ...
                    <ActionButton
                        ...
                        onClick={() => window.open(`/vm/${vm.id}/console`, '_blank')}
                    />
                </div>
                ...
                <button
                    ...
                    title={!canRequestCancel ? whyVmDenied("cancel_request") : "Solicitar cancelamento"}
                    className={`px-3 py-2 rounded-xl border text-sm flex items-center gap-2
          ${canRequestCancel ? "border-orange-500/25 bg-orange-500/10 hover:bg-orange-500/15 text-orange-100" : "border-white/10 bg-white/5 text-slate-500 opacity-60 cursor-not-allowed"}
        `}
                >
                    <Ban size={16}/>
