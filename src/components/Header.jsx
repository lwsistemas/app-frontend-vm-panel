...
function getApiModeLabel() {
    const fromStorage =
        localStorage.getItem("api_mode") || localStorage.getItem("apiMode");
...
function Chip({
    ...
    return (
        <span
            className={cls(
                compact ? "text-[10px] px-2 py-[5px]" : "text-[11px] px-2.5 py-1",
                "rounded-xl border inline-flex items-center gap-2 max-w-[280px]",
...
function HeaderButton({
    ...
    const tones = {
        slate:
            "bg-slate-900/30 border-slate-800 text-slate-200 hover:bg-slate-800/50",
        emerald:
            "bg-emerald-900/20 border-emerald-800/40 text-emerald-100 hover:bg-emerald-900/30",
        sky: "bg-sky-900/15 border-sky-800/35 text-sky-100 hover:bg-sky-900/25",
        amber:
            "bg-amber-900/15 border-amber-800/35 text-amber-100 hover:bg-amber-900/20",
    ...
    return (
        <button
            ...
            disabled={disabled}
            className={cls(
                "flex items-center gap-2 px-3 py-2 rounded-xl border transition text-xs",
                tones[tone] || tones.slate,
...
function ActionButton({
    ...
    const tones = {
        ...
        emerald:
            "bg-emerald-900/25 border-emerald-800/45 text-emerald-100 hover:bg-emerald-900/30",
        sky: "bg-sky-900/20 border-sky-800/40 text-sky-100 hover:bg-sky-900/30",
        amber:
            "bg-amber-900/20 border-amber-800/40 text-amber-100 hover:bg-amber-900/25",
    ...
    return (
        <button
            ...
            className={cls(
                "flex items-center gap-2 px-3 py-2 rounded-xl border transition text-xs whitespace-nowrap",
                tones[tone] || tones.slate,
...
export default function Header() {
    ...
    const [liveMode, setLiveMode] = useState(
        () => localStorage.getItem("header_live_mode") === "1"
    );
    ...
    const health = useMemo(() => {
        ...
        if (
            staleIpSyncCount > 0 ||
            missingSubnetCount > 0 ||
            missingGatewayCount > 0 ||
            missingNetmaskCount > 0
        ) {
    ...
    const lastSyncAgo = useMemo(
        () => formatAgo(toSecondsAgo(summary?.alerts?.lastSyncAt)),
        [summary?.alerts?.lastSyncAt]
    );
    ...
    const globalChips = useMemo(() => {
        ...
        const out = [
            { key: "health", label: "Health", value: health.label, tone: health.tone, icon: <ShieldAlert className="w-3.5 h-3.5" /> },
            { key: "online", label: "Online", value: online, tone: "emerald" },
            { key: "offline", label: "Offline", value: offline, tone: offline > 0 ? "amber" : "slate" },
            { key: "deleted", label: "Deleted", value: deleted, tone: deleted > 0 ? "amber" : "slate" },
            { key: "unknown", label: "Unknown", value: unknown, tone: unknown > 0 ? "amber" : "slate" },
    ...
    const vmSecondaryChips = useMemo(() => {
        ...
        const ramGb =
            vmCtx.memory_mb != null ? `${Math.round(vmCtx.memory_mb / 1024)} GB` : "—";
    ...
    const pageBlock = useMemo(() => {
        ...
        if (path.startsWith("/vms/create"))
            return { title: "Criar VM", subtitle: "Provisionamento", tone: "sky", status: null };
        if (path.startsWith("/vms"))
            return { title: "VMs", subtitle: "Gerenciamento", tone: "sky", status: null };
        if (path.startsWith("/inventory"))
            return { title: "Inventory DC", subtitle: "Infra / IPs", tone: "sky", status: null };
        if (path.startsWith("/users"))
            return { title: "Usuários", subtitle: "Admin", tone: "sky", status: null };
        if (path.startsWith("/invoices"))
            return { title: "Invoices", subtitle: "Financeiro", tone: "sky", status: null };
    ...
    const quickActions = useMemo(() => {
        ...
        if (isVmDetail) {
            ...
            arr.push({ label: "Dashboard", icon: <LayoutDashboard size={14} />, onClick: () => navigate("/dashboard"), tone: "slate" });
            if (isPrivileged) arr.push({ label: "Inventory", icon: <Server size={14} />, onClick: () => navigate("/inventory"), tone: "slate" });
            return arr;
        }

        const path = location.pathname || "/";
        if (path.startsWith("/vms")) {
            arr.push({ label: "Criar VM", icon: <Plus size={14} />, onClick: () => navigate("/vms/create"), tone: "emerald" });
            arr.push({ label: "Dashboard", icon: <LayoutDashboard size={14} />, onClick: () => navigate("/dashboard"), tone: "slate" });
            if (isPrivileged) arr.push({ label: "Inventory", icon: <Server size={14} />, onClick: () => navigate("/inventory"), tone: "slate" });
            return arr;
        }
        ...
        if (path.startsWith("/dashboard") || path === "/") {
            arr.push({ label: "VMs", icon: <Cpu size={14} />, onClick: () => navigate("/vms"), tone: "slate" });
            if (isPrivileged) {
                arr.push({ label: "Inventory", icon: <Server size={14} />, onClick: () => navigate("/inventory"), tone: "slate" });
        ...
        if (path.startsWith("/inventory")) {
            arr.push({ label: "Dashboard", icon: <LayoutDashboard size={14} />, onClick: () => navigate("/dashboard"), tone: "slate" });
            arr.push({ label: "VMs", icon: <Cpu size={14} />, onClick: () => navigate("/vms"), tone: "slate" });
            if (isPrivileged) arr.push({ label: "Users", icon: <Users size={14} />, onClick: () => navigate("/users"), tone: "slate" });
            return arr;
        }
    ...
    return (
        <header
            className={cls(
                ...
                "bg-gradient-to-b from-slate-950/92 via-slate-950/80 to-slate-950/65",
                "backdrop-blur-xl"
            )}
        >
            {/* linha viva */}
            ...
            <div className="px-6 h-[60px] flex items-center justify-between">
                    ...
                    >
                        <div
                            className={cls(
                                ...
                                "group-hover:bg-slate-900/80 transition"
                            )}
                        >
              ...
              <span className="text-sm font-semibold text-slate-100 truncate max-w-[520px]">
                {pageBlock.title}
              </span>
                  ...
                  {pageBlock.status}
                        ...
                        <div className="text-[11px] text-slate-400 truncate max-w-[720px]">
                            {pageBlock.subtitle}
                            {summaryErr ? (
                ...
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    {isPrivileged && (
                        <button
                            ...
                            className={cls(
                                "hidden md:flex items-center gap-2 px-3 py-2 rounded-xl border transition text-xs",
                                liveMode
                                    ? "border-emerald-700/40 bg-emerald-900/20 text-emerald-200"
                                    : "border-slate-800 bg-slate-900/30 text-slate-300 hover:bg-slate-800/50"
                            )}
                    ...
                    {isPrivileged && (
                        <div className="relative" ref={alertsRef}>
                            ...
                            >
                                <Bell size={18} />
                                {unreadCount > 0 && (
                                    <span className="absolute top-1 right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-red-600 text-[10px] flex items-center justify-center">
                    {unreadCount > 99 ? "99+" : unreadCount}
                            ...
                            {openAlerts && (
                                <div className="absolute right-0 top-12 w-96 bg-slate-950/95 border border-slate-800 rounded-2xl shadow-2xl z-50 overflow-hidden backdrop-blur-xl">
                                    <div className="px-4 py-3 text-xs text-slate-400 border-b border-slate-800 flex justify-between items-center">
                    ...
                    <div className="relative" ref={userRef}>
                        ...
                        {openUser && (
                            <div className="absolute right-0 top-14 w-56 bg-slate-950/95 border border-slate-800 rounded-2xl shadow-2xl z-50 overflow-hidden backdrop-blur-xl">
                  ...
                  Contexto da VM indisponível.
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            {vmActionRules.blockedReason && (
                                <span className="text-[11px] text-red-300 border border-red-800/40 bg-red-900/10 px-3 py-2 rounded-xl">
                  {vmActionRules.blockedReason}
                </span>
                            )}
                            ...
                            <ActionButton
                                ...
                                onClick={() => {
                                    ...
                                    setTimeout(
                                        () => setVmActionLoading((p) => ({ ...p, console: false })),
                                        600
                                    );
                                }}
                            />
